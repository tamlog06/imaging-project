...
def pyramids(image):
  ...
  for i in range(len(G) - 1):
    inter_G = interpolate(G[i+1])
    G[i] = cv2.resize(G[i], (inter_G.shape[1], inter_G.shape[0]))
...
def reconstruct(pyramid):
  ...
  for i in range(1, len(revPyramid)):
    stack = interpolate(stack) + cv2.resize(revPyramid[i], (stack.shape[1], stack.shape[0])) # upsampling simultaneously
  return stack
...
if __name__ == '__main__':
    apple = imageio.imread('../image/1.jpg')
    orange = imageio.imread('../image/source.png')
